...
<div>Import events from .json file [<!>This feature will replace existed events that edited after export the .json].</div>
...
<div>Импорт событий из файла .json [<!>Если события были отредактированы после экспорта .json, они будут заменены].</div>
...
<div>ייבוא אירועים מקובץ .json [<!>אם האירועים נערכו לאחר הייצוא של קובץ .json, הם יוחלפו].</div>
